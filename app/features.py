...
def analyze_edges_and_corners(card_image: np.ndarray) -> dict:
    ...
    edge_density_component = min(1.0, border_edge_density / 0.65)
    border_std_component = min(1.0, border_std / 55.0)
    ...
    for patch in corner_regions:
        ...
        patch_score = _clamp(
            0.65 * min(1.0, patch_edge_density / 0.70) + 0.35 * min(1.0, patch_std / 60.0),
            0.0,
            1.0,
        )
